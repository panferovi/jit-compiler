...
TEST(PEEPHOLE_OPT, ConstFoldingWithPhi)
{
    ...
    [[maybe_unused]] auto *v4 = irBuilder.CreateBr(bb2);

    irBuilder.SetInsertionPoint(bb2);
    irBuilder.CreateCondBr(v0, bb3, bb4);

    irBuilder.SetInsertionPoint(bb3);
    auto *v5 = irBuilder.CreateShl(v2, v2);
    auto *v6 = irBuilder.CreateXor(v3, v5);
    [[maybe_unused]] auto *v7 = irBuilder.CreateBr(bb4);

    irBuilder.SetInsertionPoint(bb4);
    auto *v8 = irBuilder.CreatePhi(ir::ResultType::S32);
    [[maybe_unused]] auto *v9 = irBuilder.CreateRet(v8);

    v8->ResolveDependency(v6, bb3);
    v8->ResolveDependency(v1, bb3);
